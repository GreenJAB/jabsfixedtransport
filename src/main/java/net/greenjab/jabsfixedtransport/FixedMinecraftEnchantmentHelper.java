...
public class FixedMinecraftEnchantmentHelper {
    ...
    public static int getEnchantmentCapacity(ItemStack itemStack) {
        ...
        if (!JabsFixedTransport.ItemCapacities.containsKey(item))
            if (JabsFixedTransport.SERVER!=null) {
                HashMap<Item, Integer> map = new HashMap<>(Map.of());
                map.putAll(JabsFixedTransport.ItemCapacities);
                map.put(item, getNewEnchantmentCapacity(itemStack));
                JabsFixedTransport.ItemCapacities = map;
            }
        return JabsFixedTransport.ItemCapacities.getOrDefault(item, 0);
    }
    ...
    public static List<EnchantmentInstance> getPossibleEntries(ItemStack stack) {
        ...
        Level world =  Objects.requireNonNull(JabsFixedTransport.SERVER).overworld();
